...
function calculateSum(n) {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    sum += i; // Додати поточне значення i до змінної sum
  }
  return sum; // Повернути значення змінної sum
}
...
function isPrimeNumber(num) {
  if (num <= 1) {
    return false; // Якщо число num менше або дорівнює 1, то воно не є простим, тому повертаємо false
  } else {
...
function checkDataType(value) {
  const result = typeof value; // Отримуємо тип значення value за допомогою оператора typeof
  console.log(`Значення ${value} має тип ${typeof value}`); // Виводимо у консоль повідомлення, яке містить значення value та його тип
  return result; // Повертаємо тип значення value
}
...
function findMax(num1, num2) {
  if (num1 > num2) {
    return num1; // Порівнюємо num1 та num2
...
function memoizedArea() {
  let prevSide;
  let prevResult; // Ініціалізуємо примітивні змінні для зберігання попереднього аргументу та результату
...
const createCompose = compose(increment, double); // Створюємо змінну createCompose якій присвоємо результат виконання функції compose з аргументами increment та double
